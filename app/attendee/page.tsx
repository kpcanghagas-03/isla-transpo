...
export default function AttendeePage() {
  ...
  // Desktop mouse-drag support — mirrors the touch handlers above so the
  // carousel is swipeable with a mouse, not just on touchscreens.
  ...
  // ── "Next trip" awareness ──
  // Parses display strings like "3:00 AM" or "12:00 NN" into minutes-since-midnight
  // so we can compare against the current Philippine time.
  const parseTimeToMinutes = (raw: string): number => {
    const t = raw.trim().toUpperCase();
    if (t.endsWith("NN")) return 12 * 60; // "12:00 NN" = noon
  ...
  const [nowMinutes, setNowMinutes] = useState<number>(() => getManilaMinutesNow());

  function getManilaMinutesNow(): number {
    const parts = new Date().toLocaleTimeString("en-PH", {
  ...
  // Refresh "now" every minute so the next-trip badge stays accurate
  // without needing a page reload.
  useEffect(() => {
    const interval = setInterval(() => setNowMinutes(getManilaMinutesNow()), 60_000);
  ...
  const TripCard: FC<TripCardProps> = ({ route, trips, icon, gradientFrom, gradientTo }) => {
    const nextIndex = useMemo(() => {
      return trips.findIndex((trip) => {
        const mins = parseTimeToMinutes(trip.time);
        return mins >= 0 && mins >= nowMinutes;
      });
    }, [trips, nowMinutes]);
          ...
          {trips.map((trip, index) => {
            ...
            return (
              <div
                key={index}
                style={{
                  background: isNext ? "#FFF7ED" : "#F8FAFC",
                  border: isNext ? `1.5px solid ${gradientFrom}` : "1px solid #E2E8F0",
                  borderRadius: 14,
                  padding: 12,
              ...
              >
                ...
                <div style={{ marginBottom: 6 }}>
                  <p style={{ fontSize: 11, color: "#64748B", margin: 0 }}>Departure</p>
                ...
                {/* Middle Row */}
                <div style={{ marginBottom: 6 }}>
                  <p style={{ fontSize: 11, color: "#64748B", margin: 0 }}>Vessel</p>
                ...
                {/* Bottom Row */}
                <div>
                  <p style={{ fontSize: 11, color: "#64748B", margin: 0 }}>Arrival</p>
                  <p style={{ fontSize: 15, fontWeight: 700, margin: 0, color: "#0F172A" }}>
                    📍 {trip.arrival}
                  </p>
                </div>
